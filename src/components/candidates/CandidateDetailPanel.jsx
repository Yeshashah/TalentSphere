    ...
    <div className="h-full overflow-y-auto p-6">
      ...
      <div className="flex items-start gap-4 mb-4">
        {candidate.avatar_url ? (
          <img src={candidate.avatar_url} alt="" className="w-16 h-16 rounded-2xl object-cover border-2 border-white shadow" />
        ) : (
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-slate-500 text-sm mt-0.5">{candidate.job_title}</p>
      ...
      {user?.role === 'company' && (
        <div className="flex items-center gap-2 mb-4 pb-4 border-b">
          <Button onClick={() => navigate(`/Messages?to=${candidate.user_email}`)} className="gap-2 bg-indigo-600 hover:bg-indigo-700">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {candidate.availability && <Badge variant="outline">{availLabels[candidate.availability] || candidate.availability}</Badge>}
        {candidate.open_to_work && <Badge className="bg-green-50 text-green-700 border-green-200">Open to Work</Badge>}
      ...
      {candidate.bio && (
        <div className="mb-5">
          <h4 className="font-semibold text-slate-800 mb-1.5 text-sm">About</h4>
      ...
      {candidate.skills?.length > 0 && (
        <div className="mb-5">
          <h4 className="font-semibold text-slate-800 mb-2 text-sm">Skills</h4>
      ...
      {candidate.tech_stack?.length > 0 && (
        <div className="mb-5">
          <h4 className="font-semibold text-slate-800 mb-2 text-sm">Tech Stack</h4>
          <div className="flex flex-wrap gap-1.5">
      ...
      {candidate.education_degree && (
        <div className="mb-5">
          <h4 className="font-semibold text-slate-800 mb-2 text-sm">Education</h4>
      ...
      <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t">
        {candidate.linkedin && (
          <a href={candidate.linkedin} target="_blank" rel="noopener noreferrer">
            <Button variant="outline" size="sm" className="gap-1.5"><ExternalLink className="w-3 h-3" /> LinkedIn</Button>
          </a>
        )}
        {candidate.portfolio && (
          <a href={candidate.portfolio} target="_blank" rel="noopener noreferrer">
            <Button variant="outline" size="sm" className="gap-1.5"><ExternalLink className="w-3 h-3" /> Portfolio</Button>
          </a>
        )}
        {candidate.resume_url && (
          <a href={candidate.resume_url} target="_blank" rel="noopener noreferrer">
            <Button variant="outline" size="sm" className="gap-1.5"><ExternalLink className="w-3 h-3" /> Resume</Button>
          </a>
        )}
      </div>
    </div>
